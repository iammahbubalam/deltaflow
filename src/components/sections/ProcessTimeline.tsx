    ...
    <section className="bg-white py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="absolute left-[19px] top-0 bottom-0 w-0.5 bg-[--color-gray-200] md:left-1/2 md:-ml-0.5" />
          ...
          <div className="space-y-12">
            {phases.map((phase, index) => (
              <div key={phase.name} className={cn("relative flex flex-col md:flex-row gap-8", 
                ...
                {/* Timeline Dot */}
                <div className="absolute left-0 top-0 mt-1.5 h-10 w-10 flex items-center justify-center rounded-full border-4 border-white bg-[--brand-green] text-white font-bold md:left-1/2 md:-ml-5 z-10 shadow-sm">
                  {index + 1}
                </div>
                ...
                <div className={cn("ml-16 md:ml-0 md:w-1/2", 
                  ...
                  <div className={cn("bg-[--color-gray-50] p-6 rounded-2xl border border-[--color-gray-100] hover:border-[--brand-green]/20 transition-colors",
                     index % 2 === 0 ? "md:mr-5" : "md:ml-5" // Spacing from center line
                  )}>
                    <div className={cn("flex items-center gap-2 mb-2", 
                      index % 2 === 0 ? "md:justify-end" : ""
                    )}>
                      <span className="text-sm font-bold text-[--brand-green] uppercase tracking-wider">{phase.duration}</span>
                    </div>
                    <h3 className="text-xl font-bold text-[--color-gray-900] mb-2">{phase.name}</h3>
                    ...
                    {showDetails && phase.activities && (
                      <div className="mt-4 pt-4 border-t border-[--color-gray-200]">
                        <h4 className="text-sm font-semibold text-[--color-gray-900] mb-2">Activities</h4>
